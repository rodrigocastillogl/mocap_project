...
if __name__ == '__main__':
    ...
    weights_names = [ f'weights_{len(selected_list[0])}joints.bin',
                      f'weights_{len(selected_list[1])}joints.bin',
                      'weights_fullskeleton.bin']

    training_files_names = [ f'training_{len(selected_list[0])}joints.csv',
                             f'training_{len(selected_list[1])}joints.csv',
                             'training_fullskeleton.csv' ]

    test_files_names = [ f'test_{len(selected_list[0])}joints.csv',
    ...
    for i in range( len(selected_list) ):
        ...
        # ----------------- Define model -----------------
        model = PoseNetworkShortTerm( prefix_length = 50 , selected_joints = selected_list[i] )
